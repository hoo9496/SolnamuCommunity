...
public interface ReplyMapper {
	...
	final String INSERT_REPLY = "insert into reply(id,bno,rno,content,writer,regDate) values(#{id},#{no},#{rno},#{content},#{writer},sysdate())";
	...
	final String INSERT_REREPLY = "insert into rereply(id,bno,rno,sno,content,writer,regDate) values(#{id},#{no},#{rno},#{sno},#{content},#{writer},sysdate())";
	
	final String DELETE_REREPLY = "update rereply set content='[삭제된 메세지입니다]' where bno = #{no} and rno = #{rno} and sno = #{sno}";

	final String DELETE_REPLY = "update reply set content='[삭제된 메세지입니다]' where bno = #{no} and rno = #{rno}" ;
