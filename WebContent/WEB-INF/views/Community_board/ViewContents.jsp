...
<script type="text/javascript">
	...
	function boardmodify(no,offset,pagelink){
		...
		document.location.href = "community_board_update_form.do?no="+no+"&offset="+offset+"&pagelink="+pagelink;
	...
	function boarddelete(no){
		...
		document.location.href = "community_board_delete.do?no="+no;
		}else{
			alert("삭제권한이 없습니다.");
		}
	}
	...
	function backlist(offset, pagelink){
		document.location.href = "/community_board_list.do?offset="+offset+"&pagelink="+pagelink;
	}
	...
	function deletereply(rno,id) {
		...
		if( ${"sessionScope.id" == id || sessionScope.admin == 1 }){
			document.location.href = "/community_delete_reply.do?no="+no+"&rno="+rno+"&offset="+offset+"&pagelink="+pagelink;
		}else{
			alert("삭제권한이 없습니다.");
		}
	...
	function deleterereply(rno,sno,id) {
		...
		var pagelink = ${pagelink}
		if( ${"sessionScope.id" == id || sessionScope.admin == 1 }){
			document.location.href = "/community_delete_rereply.do?no="+no+"&rno="+rno+"&sno="+sno+"&offset="+offset+"&pagelink="+pagelink;
		}else{
			alert("삭제권한이 없습니다.");
		}
...
<body>
	...
	<div id="banner"
		style="background-image: url('/resources/img/community_banner.png')">
		<div id="banner_label">
	...
	<c:if test="${!empty sessionScope.id}">
		<div id="subject">
			<c:out value="[${boardData.getKategorie()}]" /><c:out value="${boardData.getSubject()}" />
		</div> 
		...
		<form action="/community_board_reply.do" method="POST" >
			<table id="buttonTable">
				...
				<tr>
					...
					<td><input type="button" onclick="submit2(this.form)"
						value="Comment"></td>
				</tr>
			...
			<table id="replytable">
				<c:forEach var="reply" items="${replyData}">
					...
					<tr id="InputTR_${reply.rno}" style="display: none;" >
							...
							<form class="re_form" action="community_board_rereply.do" method="post" >
					...
					<c:forEach var="rereply" items="${rereplyData}">
						<c:if test="${reply.getRno() == rereply.getRno() }">
							<tr id="rereplytr"><td></td>
								...
								<td colspan="2"><c:out value="${rereply.getContent()}" /></td>
